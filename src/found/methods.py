...
def reg_gp(
    ...
    return (
        get_case_proba(
            model.predict_proba(Z),  # pyright: ignore[reportArgumentType]
...
def score_phatdiff_emd(
    ...
    """
    implements a heuristic to score label adjustment based on the distribution difference (using the earth mover's distance) between p_hat values.
    specifically two comparisons are run, and the return value is a weighted difference of:
...
def score_nulldist(Y: NumArr, V: BoolArr, pipeline_algo: Pipeline, n_iters: int = 10, **kwargs) -> NumericScalar:
    """
    implements a heuristic to score label adjustment based on the earth mover's distance between p_hat values generated by the pipeline and
    ...
    :return: earth mover's distance (as computed by scipy.stats.wasserstein_distance) between pipeline generated p_hat values and p_hat values yielded by re-running regression on randomly permuted condition labels
    """
